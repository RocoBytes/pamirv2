...
function verifyDbTargetOrExit() {
    guardVerifyDbTargetOrExit({
        prefix: '[create-user]',
        honorAllowAny: true,
        failureMessage: '[create-user] Comando abortado: DATABASE_URL no coincide con la base de datos de v2 declarada en ' +
...
async function run() {
    ...
    if (existing && existing.organizationId !== organization.id) {
        console.error(`[create-user] El usuario con email="${email}" pertenece a otra organización. ` +

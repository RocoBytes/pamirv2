...
const KINDS = Object.keys(MAIL_FROM);
async function main() {
    ...
    if (noSmtp.length > 0) {
        console.error(`[test-email] Este script exige el proveedor "smtp" para cada tipo de correo. Resolvieron a "console": ` +
            `${noSmtp.join(', ')}. Define SMTP_HOST (y el resto de las variables SMTP_*) antes de correrlo.`);
        process.exitCode = 1;
        return;
    }
    const organization = await runAsPlatform(() => prisma.organization.findUniqueOrThrow({ where: { slug: 'pamir' } }));
    const results = [];

...
const OBJECT_KEY_PATTERN = new RegExp(`^orgs/(${ORG_ID_FRAGMENT})/(${FILE_KINDS.join('|')})/${UUID_FRAGMENT}\\.[a-z0-9]{1,5}$`);
 ...
 */
export function buildObjectKey({ organizationId, kind, extension }) {
...
export function objectKeyBelongsTo(key, organizationId) {
    if (!isObjectKey(key))
        return false;
    if (!ORGANIZATION_ID_PATTERN.test(organizationId))
        return false;
...
export function organizationPrefix(organizationId) {
    if (!ORGANIZATION_ID_PATTERN.test(organizationId)) {
 ...
 * bucket completo.
 */
export function assertSafeObjectPrefix(prefix) {

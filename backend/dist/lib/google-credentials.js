...
function describirErrorGoogle(err) {
    ...
    if (data && typeof data === 'object') {
        const { error, error_description: descripcion } = data;
...
function construirCliente(refreshToken) {
...
async function resolverToken() {
    try {
        ...
        if (fila)
            return { token: decryptSecret(fila.value), origen: 'db' };
    }
    catch (err) {
        ...
        console.error('[google-credentials] No se pudo leer el token guardado:', err instanceof Error ? err.message : err);
    }
 ...
 */
export function invalidateGoogleCredentials() {
 ...
 */
export async function probarRefreshToken(refreshToken) {
    const client = refreshToken === undefined ? await getOAuth2Client() : construirCliente(refreshToken);
    try {
        const { token } = await client.getAccessToken();
        if (!token)
            return { ok: false, motivo: 'Google no devolvió un access token.' };
...
export async function getEstadoCredencial() {
    const fila = await prisma.appSecret.findUnique({
        where: { key: GOOGLE_REFRESH_TOKEN_KEY },
    });
    const origen = fila ? 'db' : 'env';
...
export async function guardarRefreshToken(refreshToken, actualizadoPor) {

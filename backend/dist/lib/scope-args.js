...
const TENANT_MODEL_SET = TENANT_MODELS;
const GLOBAL_MODEL_SET = GLOBAL_MODELS;
 ...
 */
export function scopeArgs(params) {
    const { model, operation, store } = params;
    const args = params.args ?? {};
    if (store === undefined) {
        throw new TenantContextError(`"${model}.${operation}" se ejecutó sin ningún contexto de tenant activo (falta runWithOrganization o runAsPlatform). ` +
    ...
    if (!TENANT_MODEL_SET.includes(model)) {
        throw new TenantContextError(`Modelo desconocido para el aislamiento multi-club: "${model}". Agrégalo a TENANT_MODELS, GLOBAL_MODELS ` +
...
function scopeTenantModel(model, operation, args, organizationId) {
    ...
    throw new TenantContextError(`Operación "${operation}" sobre "${model}" no está contemplada por el aislamiento multi-club. Revisa ` +
...
function scopeOrganizationSelf(operation, args, organizationId) {
    const opsConWhere = new Set([...READ_DELETE_OPS, ...UPDATE_OPS]);
    if (opsConWhere.has(operation)) {
        const scoped = { ...args, where: scopeOrganizationWhere(args['where'], organizationId, operation) };
        if (UPDATE_OPS.has(operation)) {
    ...
    throw new TenantContextError(`Operación "${operation}" sobre "Organization" no está permitida dentro de un contexto de club: un club ` +
        'solo puede leer o actualizar su propia fila. Usa runAsPlatform para administrar organizaciones.');
}
function scopeOrganizationWhere(where, organizationId, operation) {
    const whereObj = isPlainObject(where) ? { ...where } : {};
    if ('id' in whereObj && whereObj['id'] !== organizationId) {
        throw new TenantContextError(`"Organization.${operation}" intentó filtrar por id="${String(whereObj['id'])}" dentro del contexto ` +
...
function scopeWhere(where, organizationId, model, operation) {
    const whereObj = isPlainObject(where) ? { ...where } : {};
    if ('organizationId' in whereObj && whereObj['organizationId'] !== organizationId) {
        throw new TenantContextError(`"${model}.${operation}" intentó filtrar por organizationId="${String(whereObj['organizationId'])}" ` +
...
function scopeCreateData(data, organizationId, model, operation) {
    if (!isPlainObject(data)) {
        throw new TenantContextError(`"${model}.${operation}" no incluyó "data" con organizationId.`);
    }
    if (data['organizationId'] !== organizationId) {
        throw new TenantContextError(`"${model}.${operation}" debe crear con organizationId="${organizationId}" (el club del contexto ` +
...
function scopeUpdateData(data, organizationId, model, operation) {
    if (!isPlainObject(data)) {
        return data;
    }
    if ('organizationId' in data && data['organizationId'] !== organizationId) {
        throw new TenantContextError(`"${model}.${operation}" intentó mover la fila al club "${String(data['organizationId'])}": una fila ` +

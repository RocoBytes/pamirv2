...
function boolRow(label, value, detail) {
...
function clausulaBlock(number, title, body, accepted) {
    const icon = accepted ? '&#10003;' : '&#10007;';
...
// ─── Utilities ────────────────────────────────────────────────────────────────
function opt(value, fallback = 'No especificado') {
...
function safeParticipantNames(v) {
    if (!Array.isArray(v))
        return [];
    return v.map((item) => {
...
function evaluacionCtaBlock(evaluacionUrl) {
    return `
        <tr>
...
export function buildSalidaNotificationEmail(nombreCompleto, salida, branding) {
  ...
  `;
    return emailShell('Registro en salida de montaña', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, '', branding);
}
// ─── Cierre notification ──────────────────────────────────────────────────────
export function buildCierreNotificationEmail(nombreCompleto, salida, cierre, branding, evaluacionUrl) {
  ...
  `;
    return emailShell('Cierre de salida de montaña', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, (evaluacionUrl ? evaluacionCtaBlock(evaluacionUrl) : '') + feedbackCierreBlock(branding), branding);
}
        ...
        <tr>
          <td style="padding:0 32px 24px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${BORDER};border-radius:6px;overflow:hidden;border-collapse:collapse;">
              ...
              ${clausulaBlock('1', 'Declaración de Salud y Aptitud Física', 'El firmante declara que se encuentra en condiciones físicas y psíquicas aptas para la práctica de deportes de montaña (senderismo, escalada, montañismo y otras actividades relacionadas). Declara que la información proporcionada en este formulario es veraz y completa, asumiendo que la ocultación de antecedentes médicos puede comprometer su seguridad y la del grupo.', data.declaracionSalud)}
              ${clausulaBlock('2', 'Aceptación de Riesgo y Exención de Responsabilidad', 'Reconozco que las actividades de montaña son intrínsecamente riesgosas y pueden implicar peligros derivados del terreno, clima extremo, caída de rocas, fallas de equipo y otros factores objetivos y subjetivos que pueden resultar en lesiones graves o la muerte. <strong>Exención:</strong> Libero de toda responsabilidad civil y criminal al Club, a sus directivos, guías, instructores y miembros, por cualquier accidente o incidente derivado de los riesgos propios de la actividad o de mi propia negligencia, siempre que el club haya actuado bajo los protocolos de seguridad estándar. El Club no se hace responsable por accidentes derivados de la omisión de información o negligencia de los participantes.', data.aceptacionRiesgo)}
              ${clausulaBlock('3', 'Consentimiento de Uso de Datos Personales (Ley 19.628)', 'En cumplimiento con la Ley N° 19.628 sobre Protección de la Vida Privada, autorizo expresamente al Club para: (a) Tratar mis datos personales y sensibles (salud) con el fin exclusivo de gestionar mi participación en actividades y responder ante emergencias médicas. (b) Almacenar de forma segura esta información, la cual solo será accesible por el cuerpo técnico o servicios de emergencia en caso de ser necesario. (c) Comunicar estos datos a centros de salud o cuerpos de socorro en caso de rescate o atención urgente. Los datos serán utilizados exclusivamente para la gestión de seguridad en montaña, coordinación de rescates, registro estadístico de incidentes y cumplimiento de protocolos internos del club.', data.consentimientoDatos)}
              ${clausulaBlock('4', 'Derecho de Imagen', 'Autorizo el uso de fotografías o videos capturados durante las salidas para fines promocionales o educativos del club, sin derecho a compensación económica.', data.derechoImagen)}
...
export function buildAlertaSalidaEmail(salida, branding) {
    ...
    const participantRows = participantesArr.length
        ? participantesArr
            .map((p) => `<li style="margin-bottom:2px;">${escapeHtml(p.nombre ?? '')}${p.rut ? ` <span style="color:${GRAY};font-size:12px;">(${escapeHtml(p.rut)})</span>` : ''}</li>`)
            .join('')
  ...
  `;
    return emailShell('ALERTA — Salida sin cierre registrado', intro, tabla, `Esta alerta fue generada automáticamente por el sistema ${escapeHtml(branding.shortName)}. Hora de la alerta superada sin cierre.`, '', branding);
}
...
export function buildRecordatorioCierreEmail(salida, branding) {
  ...
  `;
    return emailShell('Recordatorio — Cierra tu salida', intro, tabla, `Este recordatorio fue generado automáticamente por el sistema ${escapeHtml(branding.shortName)} antes de la hora de alerta.`, '', branding);
}
...
export function buildInvitationEmail(data, branding) {
    ...
    return emailShell(`Invitación a ${escapeHtml(branding.shortName)}`, intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}. Si no esperabas esta invitación, puedes ignorar este mensaje.`, cta, branding);
}
...
export function rangoFechasEvento(evento) {
...
export function buildEventoInscripcionConfirmadaEmail(nombre, evento, inscripcion, branding) {
  ...
  `;
    ...
    return emailShell('Postulación recibida', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, nota, branding);
}
export function buildEventoSeleccionadoEmail(nombre, evento, branding) {
  ...
  `;
    const nota = `
        ...
        ${eventoCtaBlock(branding)}`;
    return emailShell('Selección confirmada', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, nota, branding);
}
export function buildEventoNoSeleccionadoEmail(nombre, evento, resumen, branding) {
  ...
  `;
    const nota = `
        ...
        ${eventoCtaBlock(branding)}`;
    return emailShell('Resultado de tu postulación', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, nota, branding);
}
export function buildEventoCanceladoEmail(nombre, evento, branding) {
  ...
  `;
    const nota = `
        ...
        ${eventoCtaBlock(branding)}`;
    return emailShell('Evento cancelado', intro, tabla, `Este correo es una notificación automática del sistema ${escapeHtml(branding.shortName)}.`, nota, branding);
}

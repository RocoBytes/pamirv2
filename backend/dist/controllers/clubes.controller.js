...
export async function getClubMarca(req, res) {
    const slug = req.params.slug;
    try {
        const org = await runAsPlatform(() => prisma.organization.findUnique({
            where: { slug },
 ...
 */
export async function getClubLogo(req, res) {
    const slug = req.params.slug;
    try {
        const org = await runAsPlatform(() => prisma.organization.findUnique({ where: { slug }, select: { logoObjectKey: true } }));

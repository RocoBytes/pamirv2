...
export async function getStats(_req, res) {
    try {
        const [totalSalidas, porStatus, totalCierres, incidentes, accidentes, porMesRaw, topDisciplinasRaw,] = await prisma.$transaction([
            prisma.salida.count(),
      ...
      `,
        ...
        const countFor = (status) => porStatus.find((s) => s.status === status)?._count._all ?? 0;
...
function distinctSorted(values) {
    const set = new Set(values.filter((x) => typeof x === 'string' && x.trim() !== ''));
...
export async function getDashboard(req, res) {
    try {
        ...
        const club = clubRaw && MEMBRESIA_CLUBS.includes(clubRaw) ? clubRaw : undefined;
        ...
        const statuses = (statusCsv ? statusCsv.split(',') : [])
            .map((s) => s.trim())
            .filter((s) => SALIDA_STATUSES.includes(s));
        // ── Build Prisma where from scalar filters ──
        const where = {};
        if (desde || hasta) {
            const range = {};
            if (desde)
                range.gte = new Date(desde);
            if (hasta)
                range.lte = new Date(hasta);
            where.fechaInicio = range;
        }
        if (statuses.length > 0)
            where.status = { in: statuses };
        if (lider)
            where.liderCordada = lider;
        if (disciplina)
            where.disciplina = disciplina;
        if (tipoSalida)
            where.tipoSalida = tipoSalida;
        if (temporada)
        ...
        const salidaMonth = new Map();
        const estadoCount = new Map();
        const mesCount = new Map();
        const incidentesPorMesMap = new Map();
        const liderCount = new Map();
        const tipoIncidenteCount = new Map();
        const tipoAccidenteCount = new Map();
        let canceladas = 0;
        ...
        for (const s of finalSalidas) {
            ...
            if (tieneCierre)
                conCierreCount += 1;
            else if (PENDING_CLOSE_STATUSES.has(s.status))
                pendientesCierre += 1;
            ...
            if (avgCalidad !== null && avgCalidad < LOW_RATING_THRESHOLD)
                salidasEvalBaja += 1;
        }
        ...
        const promedioCalidad = finalEvaluaciones.length > 0
            ? Math.round((calidadSum / finalEvaluaciones.length) * 100) / 100
            : null;
        const sortMonthsAsc = (a, b) => a.localeCompare(b);
 ...
 */
async function buildParticipantesSalud(salidaId) {
    ...
    if (!ACTIVE_SALUD_STATUSES.includes(salida.status))
    ...
    const participantes = participantesJson.map((p) => {
        const rut = p.rut ?? '';
...
export async function getSaludSalida(req, res) {
    try {
...
export async function enviarSaludSalida(req, res) {
    try {
        const id = req.params.id;
        const result = await buildParticipantesSalud(id);
        if ('notFound' in result) {
...
export async function getGoogleCredencial(_req, res) {
    try {
...
export async function saveGoogleCredencial(req, res) {
    try {
        ...
        console.error('[saveGoogleCredencial]', error instanceof Error ? error.message : 'error desconocido');

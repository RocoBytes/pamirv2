...
// cuente como llenos.
function toEventoData(data) {
    const out = {};
    const texto = (v) => (v === '' ? null : v);
    if (data.titulo !== undefined)
        out.titulo = data.titulo;
    if (data.categoriaId !== undefined)
        out.categoriaId = data.categoriaId;
    if (data.fechaInicio !== undefined)
        out.fechaInicio = data.fechaInicio ? fechaUtc(data.fechaInicio) : null;
    if (data.fechaFin !== undefined)
        out.fechaFin = data.fechaFin ? fechaUtc(data.fechaFin) : null;
    if (data.horaInicio !== undefined)
        out.horaInicio = data.horaInicio;
    if (data.duracionTexto !== undefined)
        out.duracionTexto = texto(data.duracionTexto);
    if (data.ubicacion !== undefined)
        out.ubicacion = texto(data.ubicacion);
    if (data.reunionCoordinacion !== undefined)
        out.reunionCoordinacion = texto(data.reunionCoordinacion);
    if (data.organizadorNombre !== undefined)
        out.organizadorNombre = texto(data.organizadorNombre);
    if (data.alturaMaximaMsnm !== undefined)
        out.alturaMaximaMsnm = data.alturaMaximaMsnm;
    if (data.dificultad !== undefined)
        out.dificultad = data.dificultad;
    if (data.cupos !== undefined)
        out.cupos = data.cupos;
    if (data.fechaCorte !== undefined)
        out.fechaCorte = data.fechaCorte ? composeFechaCorte(data.fechaCorte) : null;
    if (data.objetivo !== undefined)
        out.objetivo = texto(data.objetivo);
    if (data.itinerario !== undefined)
        out.itinerario = texto(data.itinerario);
    if (data.incluye !== undefined)
        out.incluye = texto(data.incluye);
    if (data.noIncluye !== undefined)
        out.noIncluye = texto(data.noIncluye);
    if (data.recomendaciones !== undefined)
        out.recomendaciones = texto(data.recomendaciones);
    if (data.avisoDestacado !== undefined)
        out.avisoDestacado = texto(data.avisoDestacado);
    return out;
}
// E2: la fecha de término no puede ser anterior a la de inicio
function violaE2(inicio, fin) {
...
export async function updateEvento(req, res) {
    ...
    try {
        ...
        const inicio = parsed.data.fechaInicio !== undefined
            ? parsed.data.fechaInicio
                ? fechaUtc(parsed.data.fechaInicio)
                : null
            : evento.fechaInicio;
        const fin = parsed.data.fechaFin !== undefined
...
// ─── Eliminar borrador ────────────────────────────────────────────────────────
export async function deleteEventoBorrador(req, res) {
    const id = req.params['id'];
    try {
 ...
 */
async function cargarEventoParaAdjunto(req, res) {
    const id = req.params['id'];
    let evento;
    try {
 ...
 */
export async function uploadItinerarioAdjunto(req, res) {
    ...
    busboy.on('file', async (_fieldname, fileStream, info) => {
        ...
        try {
            ...
            const result = await uploadToGoogleDrive(fileStream, sanitizePronosticoFilename(rawFilename), mimeType || 'application/octet-stream', MAX_ADJUNTO_BYTES);
            let actualizado;
            try {
            ...
            if (anteriorId && anteriorId !== result.fileId) {
                deleteFromGoogleDrive(anteriorId).catch((err) => console.error('[uploadItinerarioAdjunto] Could not delete previous attachment from Drive:', err));
            }
        }
        catch (err) {
            const code = err.code;
 ...
 */
export async function deleteItinerarioAdjunto(req, res) {
    const evento = await cargarEventoParaAdjunto(req, res);
    if (!evento)
        return;
    try {
...
export async function publicarEvento(req, res) {
    const id = req.params['id'];
    try {
        const evento = await prisma.evento.findUnique({ where: { id } });
        if (!evento) {
        ...
        const fechaCorte = evento.fechaCorte;
        if (fechaSantiagoDe(fechaCorte) > evento.fechaInicio.toISOString().slice(0, 10)) {
...
export async function despublicarEvento(req, res) {
    const id = req.params['id'];
    try {
        const evento = await prisma.evento.findUnique({ where: { id } });
        if (!evento) {
...
export async function finalizarEvento(req, res) {
    const id = req.params['id'];
...
export async function reenviarNotificaciones(req, res) {
    const id = req.params['id'];
    try {
        const evento = await prisma.evento.findUnique({
            where: { id },

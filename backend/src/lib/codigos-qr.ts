...
export type QrDuracion = keyof typeof QR_DURACIONES;

export const QR_DURACION_DEFAULT: QrDuracion = '24h';
...
export function estadoCodigoQr(qr: CodigoQrEstadoInput, now: Date): EstadoCodigoQr {
  if (qr.revocadoAt) return 'REVOCADO';
  if (qr.expiresAt <= now) return 'EXPIRADO';
  if (qr.usosRestantes <= 0) return 'AGOTADO';
  return 'ACTIVO';
}

export const ROL_QR: RolUsuario = 'SOCIO';
...
export function cifrarTokenQr(token: string, secret: string): string {
...
export function descifrarTokenQr(payload: string, secret: string): string | null {
  try {

...
export type TenantStore = { kind: 'platform' } | { kind: 'org'; organizationId: string };
...
const storage = new AsyncLocalStorage<TenantStore | undefined>();
...
function runInStore<T>(store: TenantStore | undefined, fn: () => T): T {
...
export function requireOrganizationId(): string {
  ...
  if (!store || store.kind !== 'org') {
    throw new TenantContextError(
      'Se requiere un contexto de club (runWithOrganization) para esta operación y no hay ninguno activo.',
    );
  }

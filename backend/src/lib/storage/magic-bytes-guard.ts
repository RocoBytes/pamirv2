...
export interface MagicBytesGuardOptions {
  /** Cualquiera de estas cabeceras da por válido el archivo. */
  signatures: Buffer[];
  /**
   * Tolera un BOM UTF-8 y espacios iniciales antes de la firma.
   *
   * Se usa solo con formatos de texto (XML/GPX), donde los exportadores de los
   * relojes y las apps de montaña anteponen BOM o saltos de línea y el archivo
   * sigue siendo perfectamente válido. Para formatos binarios queda en false:
   * ahí la cabecera está en el byte 0 y no hay ambigüedad.
   */
...
export class MagicBytesGuard extends Transform {
  ...
  private readonly signatures: Buffer[];
  private readonly tolerant: boolean;
  private readonly needed: number;

  constructor(options: MagicBytesGuardOptions = { signatures: [PDF_SIGNATURE] }) {
    super();
  ...
  private contentStart(buffer: Buffer): number {

...
export type StorageProviderName = 'gcs' | 'memory';

export interface SelectStorageProviderParams {
  storageProvider: string | undefined;
  gcsBucket: string | undefined;
  gcsProjectId: string | undefined;
  gcsCredentialsJson: string | undefined;
...
function assertGcsEnv(params: SelectStorageProviderParams): void {
  const missing: string[] = [];
  if (!params.gcsBucket) missing.push('GCS_BUCKET');
  if (!params.gcsProjectId) missing.push('GCS_PROJECT_ID');
  if (!params.gcsCredentialsJson) missing.push('GCS_CREDENTIALS_JSON');
...
export function selectStorageProvider(params: SelectStorageProviderParams): StorageProviderName {
  ...
  const resolved: StorageProviderName =
    explicit === 'gcs' ? 'gcs' : explicit === 'memory' ? 'memory' : params.gcsBucket ? 'gcs' : 'memory';
  ...
  if (resolved === 'memory' && params.nodeEnv === 'production') {
    throw new Error(
      'No se puede usar el proveedor de almacenamiento "memory" en producción: los archivos desaparecerían en ' +
...
export function parseGcsCredentials(base64: string): ParsedGcsCredentials {
  ...
  const candidate = parsed as { client_email?: unknown; private_key?: unknown } | null;
...
export function getFileStorage(): FileStorage {
  ...
  cachedStorage =
    name === 'gcs'
      ? createGcsStorage({
          bucket: params.gcsBucket as string,
          projectId: params.gcsProjectId as string,
          credentials: parseGcsCredentials(params.gcsCredentialsJson as string),
        })
      : createMemoryStorage();

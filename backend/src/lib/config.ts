...
export function resolveFrontendUrl(env: Record<string, string | undefined>): string {
...
export function mailAccountEnvVarNames(kind: EmailKind): { userEnvVar: string; passEnvVar: string } {
...
export function resolveMailAccounts(env: Record<string, string | undefined>): Record<EmailKind, MailAccount> {
  const addresses = resolveMailFrom(env);
  const globalUser = env['SMTP_USER']?.trim();
  const globalPass = env['SMTP_PASS']?.trim();

  const result = {} as Record<EmailKind, MailAccount>;
...
export const MAIL_ACCOUNTS: Record<EmailKind, MailAccount> = resolveMailAccounts(process.env);

...
// upload.route.ts importa el controlador, que a su vez importa
// src/lib/prisma.ts: lanza al importarse si DATABASE_URL no está definida
// (estos tests no cargan dotenv). No se abre ninguna conexión real: prisma.ts
// solo construye el adaptador de pg (perezoso hasta la primera query), y
// estos tests nunca ejecutan una. El import dinámico evita que el módulo se
// cargue antes de fijar esta variable.
...
async function loadRouterStack(): Promise<StackLayer[]> {
  const { default: router } = await import('./upload.route.js');
...
describe('upload.route — cierre de acceso anónimo', () => {
  it('todas las rutas exigen requireAuth en su cadena de handlers', async () => {
    ...
    assert.ok(routeEntries.length > 0, 'se esperaba encontrar rutas registradas en upload.route.ts');

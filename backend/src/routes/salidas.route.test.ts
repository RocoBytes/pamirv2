...
// salidas.route.ts importa el controlador, que a su vez importa
...
async function loadRouterStack(): Promise<StackLayer[]> {
  const { default: router } = await import('./salidas.route.js');
...
describe('salidas.route — cierre de acceso anónimo', () => {
  it('todas las rutas exigen requireAuth en su cadena de handlers', async () => {
    ...
    assert.ok(routeEntries.length > 0, 'se esperaba encontrar rutas registradas en salidas.route.ts');
  ...
  it('no expone /:id/claim en ningún método (flujo de guest eliminado)', async () => {
    const stack = await loadRouterStack();
    const claimLayers = stack.filter((layer) => layer.route?.path === '/:id/claim');
    assert.deepEqual(claimLayers, []);
  });
});

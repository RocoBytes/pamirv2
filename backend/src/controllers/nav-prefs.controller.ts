...
const TAB_KEYS = ['inicio', 'eventos', 'documentos', 'contactos'] as const;
const QUICK_KEYS = ['contactos', 'eventos', 'documentos', 'admin', 'invitar', 'integrante'] as const;
 ...
 */
const PINNED_TABS: TabKey[] = ['inicio', 'contactos'];
 ...
 */
export function sanitizeNavPreferences(input: NavPreferences): NavPreferences {
  ...
  for (const pinned of PINNED_TABS) {
    if (tabs.includes(pinned)) continue;
    if (pinned === 'inicio') tabs.unshift(pinned);
    else tabs.push(pinned);
  }
...
export async function getNavPreferences(req: Request, res: Response): Promise<void> {
  try {
...
export async function saveNavPreferences(req: Request, res: Response): Promise<void> {
  try {
    ...
    const saved = await prisma.dashboardLayout.upsert({
      ...
      create: {
        ...
        layout: preferences as unknown as Prisma.JsonObject,
      },
      update: { layout: preferences as unknown as Prisma.JsonObject },
    });
...
export async function deleteNavPreferences(req: Request, res: Response): Promise<void> {
  try {

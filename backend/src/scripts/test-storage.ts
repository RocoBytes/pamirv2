...
async function main(): Promise<void> {
  ...
  if (resolved !== 'gcs') {
    console.error(
      '[test-storage] Este script ejercita el bucket real de GCS: exige STORAGE_PROVIDER=gcs (o las variables ' +
  ...
  try {
    ...
    await check('un upload que excede maxBytes se rechaza con FILE_TOO_LARGE', async () => {
      ...
      try {
        ...
        if (code !== 'FILE_TOO_LARGE') throw new Error(`código de error inesperado: ${code}`);
      }
      if (!threw) throw new Error('se esperaba que el upload lanzara');
    });

...
import { parseTenantArgs, type CreateTenantArgs, type UpdateTenantArgs } from './tenant-args.js';
import {
  crearClub,
  listarClubes,
  cambiarEstadoClub,
...
function verifyDbTargetOrExit(): void {
  guardVerifyDbTargetOrExit({
    ...
    honorAllowAny: true,
    failureMessage:
      '[tenant] Comando abortado: DATABASE_URL no coincide con la base de datos de v2 declarada en ' +
...
const crearInvitacionAdmin: TenantsDeps['crearInvitacionAdmin'] = (organizationId, email) =>
...
function buildTenantsDeps(): TenantsDeps {
...
function esFrontendUrlLocal(url: string): boolean {
  let hostname: string;
  try {
...
function advertirSiFrontendLocal(): void {
  if (esFrontendUrlLocal(FRONTEND_URL)) {
    console.log(
      '[tenant] ADVERTENCIA: FRONTEND_URL apunta a localhost — no envíes este enlace a un administrador real.',
    );
  }
}
...
interface ColumnaTabla {
  header: string;
  value: (row: ClubListRow) => string;
}

const COLUMNAS_LISTADO: ColumnaTabla[] = [
...
function printTable(rows: ClubListRow[]): void {
  if (rows.length === 0) {
...
async function runCreate(deps: TenantsDeps, data: CreateTenantArgs): Promise<void> {
  ...
  if (!invitacion.emitida) {
    console.error(
      `[tenant] ADVERTENCIA: el club quedó creado, pero no se pudo emitir la invitación del primer ADMIN: ${invitacion.error}`,
    );
...
async function runList(deps: TenantsDeps): Promise<void> {
  const clubes = await listarClubes(deps);
  printTable(clubes);
}

async function runCambiarEstado(deps: TenantsDeps, slug: string, nuevoEstado: OrganizationStatus): Promise<void> {
  ...
  if (estadoNuevo === 'SUSPENDED') {
    ...
    console.log(
      '[tenant] Las alertas de seguridad ("salida sin cierre") de sus salidas abiertas seguirán enviándose con normalidad.',
    );
  }
}

async function runInvite(deps: TenantsDeps, slug: string, adminEmail: string): Promise<void> {
...
const ETIQUETAS_CAMPO: Record<CampoActualizable, string> = {
  name: 'Nombre',
...
function formatValorCampo(valor: string | null): string {
  return valor === null ? '(sin nombre corto)' : valor;
}

async function runUpdate(deps: TenantsDeps, data: UpdateTenantArgs): Promise<void> {
  ...
  if (cambios.some((cambio) => cambio.campo === 'alertEmail')) {
    console.log(
      '[tenant] ADVERTENCIA: cambiaste el email de alerta de seguridad ("salida sin cierre"). No hay paso de ' +

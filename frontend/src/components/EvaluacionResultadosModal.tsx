...
function PromedioRow({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center justify-between gap-3 py-2.5 border-b border-secondary/10 last:border-b-0">
      ...
      <span className="flex items-center gap-1.5 shrink-0">
        ...
        <span className="text-sm font-bold text-primary tabular-nums">{value.toFixed(1)}</span>
      </span>
    </div>
  )
}
      ...
      >
        <div className="sticky top-0 bg-white border-b border-secondary/10 px-4 sm:px-6 py-4 flex items-start justify-between gap-3">
          ...
          <button
            onClick={onClose}
            aria-label="Cerrar"
            className="shrink-0 p-1.5 rounded-lg text-on-surface-variant hover:bg-surface-container-low focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            <X size={18} />
          </button>
        </div>
          ...
          {resultados && (
            <>
              ...
              {resultados.totalRespuestas === 0 ? (
                ...
                <>
                  <div className="rounded-xl border border-secondary/15 px-4 mb-5">
                  ...
                  {resultados.comentarios.length === 0 ? (
                    <p className="text-sm text-on-surface-variant">No se dejaron comentarios.</p>
                  ) : (
                    <ul className="flex flex-col gap-2">
                      {resultados.comentarios.map((c, i) => (
                        <li
                          key={i}
                          className="rounded-xl bg-[#f8fafc] border border-secondary/10 px-3 py-2.5 text-sm text-slate-700 whitespace-pre-wrap"
                        >
                          {c}
                        </li>
                      ))}
